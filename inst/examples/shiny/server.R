...
#this will generate MaemodSYS function which can be used in deSolve::ode
maemod.gensysfunction(input.filename = "testmodel.mod")

##for plotting the output
RunOde <- function(parms){
  #time 
  timegrid <- seq(0, 42/365, by = (1/365))
  ...
  outode <- ode(y = init, times = timegrid, func = MaemodSYS, parms = parameters, method="vode")
